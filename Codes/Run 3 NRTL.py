...
plt.plot(x_vis, x_vis, '--', label=r'$y=x$', alpha=1, color='black')
plt.xlabel('Liquid-phase Ethanol Mole Fraction, $x$', fontsize=12)
plt.ylabel('Vapor-phase Ethanol Mole Fraction, $y$', fontsize=12)
...
plt.figure(figsize=(6, 6))
plt.plot(x_W_values, x_D_values, 'r-', linewidth=2, label='Numerical stagewise calculation')
#plt.plot(x_W_unique, xw_to_xd_spline(x_W_unique), 'r--', linewidth=1.5, label='Cubic spline interpolation')
plt.scatter(x_W_vis, x_D_vis, color='black', s=100, zorder=5)
plt.xlabel('Bottoms Ethanol Mole Fraction, $x_W$', fontsize=12)
plt.ylabel('Distillate Ethanol Mole Fraction, $x_D$', fontsize=12)
...
# ---------------------------------------------------------------------
# Boiling temperature at regular intervals
print('\nBoiling Temperature vs. Time')

# Sample every 6 min from start, always include final time
times_min = time / 60
targets = np.arange(0, times_min[-1], 6)
targets = np.append(targets, times_min[-1])

# Find nearest simulation points, remove duplicates
idx = np.unique([np.abs(times_min - t).argmin() for t in targets])

# Display table
df = pd.DataFrame({
    "Time (min)": np.round(Time_to_reach_bpt + times_min[idx], 2),
    "Boiling Temperature (°C)": np.round(T_b[idx], 2)
})
print(df)
...
plt.plot(Time_to_reach_bpt + time[1:num_steps]/60, instant_vv_p[1:num_steps], linewidth=2, color='red', zorder=3)
...
for start, end, conc in zip(instant_start_times, instant_end_times, theo_inst_conc):
    plt.hlines(conc, start, end, colors='darkblue', linewidth=2, linestyles='-.', zorder=4)
# Model-experiment deviation
for start, end, exp_conc, model_conc in zip(instant_start_times, instant_end_times,
        exp_sample_conc_rt, theo_inst_conc):
    mid_time = (start + end) / 2
    plt.vlines(mid_time, ymin=min(exp_conc, model_conc),
        ymax=max(exp_conc, model_conc), colors='black', linewidth=1.5, linestyles='--',zorder=1)
...
deviation_handle = Line2D([0], [0], color='black', linewidth=1.5, linestyle='--')
plt.legend(handles=[ exp_handle, theo_inst_handle, theo_avg_handle, deviation_handle],
    labels=['Experimental (interval average)', 'Model (instantaneous)', 'Model (interval average)', 'Model-experiment deviation'])
plt.xlabel('Distillation Time (min)', fontsize=12)
plt.ylabel('Distillate Ethanol Concentration (v/v%)', fontsize=12)
...
for start, end, r in zip(instant_start_times, instant_end_times, exp_rate):
    plt.hlines(r, start, end, colors='black', linewidth=2, zorder=6)
    plt.scatter([(start+end)/2], [r], color='black', s=70, zorder=7)
plt.plot(time_minutes, total_rate,   linewidth=2, color='red',   zorder=4)
plt.plot(time_minutes, ethanol_rate, linewidth=2, linestyle='--', color='green', zorder=2)
plt.plot(time_minutes, water_rate,   linewidth=2, linestyle=':', color='blue',  zorder=3)
for start, end, r in zip(instant_start_times, instant_end_times, theo_rate):
    plt.hlines(r, start, end, colors='darkblue', linewidth=2, linestyles='-.', zorder=5)
# Model-experiment deviation
for start, end, exp_r, model_r in zip(instant_start_times, instant_end_times, exp_rate, theo_rate):
    mid_time = (start + end) / 2
    plt.vlines(mid_time, ymin=min(exp_r, model_r), ymax=max(exp_r, model_r), colors='black', linewidth=1.5, linestyles='--', zorder=1)
# Custom Legend handles
exp_handle   = Line2D([0],[0], color='black', linewidth=2, marker='o', markersize=8)
...
deviation_handle = Line2D([0], [0], color='black', linewidth=1.5, linestyle='--')
plt.legend( handles=[ exp_handle, total_handle, eth_handle, wat_handle, avg_handle, deviation_handle],
    labels=['Experimental (interval average)', 'Model (total)', 'Model (ethanol contribution)', 'Model (water contribution)',
        'Model (interval average)', 'Model–experiment deviation'])
plt.xlabel('Distillation Time (min)', fontsize=12)
plt.ylabel('Distillate Collection Rate (mL/min)', fontsize=12)
...
plt.figure(figsize=(6, 6))
plt.gca().set_axisbelow(True)
plt.scatter(exp_interval_times, exp_interval_vol, color='black', s=70, marker='o', zorder=5, label='Experimental') 
plt.plot(Time_to_reach_bpt+time[1:num_steps]/60, 1000*V_t_collected[1:num_steps], label='Model (total)', 
         linewidth=2, zorder=4, color='red', linestyle='-')
plt.plot(Time_to_reach_bpt+time[1:num_steps]/60, 1000*V_e_collected[1:num_steps], label='Model (ethanol contribution)', 
         linewidth=2,zorder=3, color='green', linestyle='--')
plt.plot(Time_to_reach_bpt+time[1:num_steps]/60, 1000*V_w_collected[1:num_steps], label='Model (water contribution)', 
         zorder=2, linewidth=2, color='blue', linestyle=':')
# Model–experiment deviation
# Model time and cumulative volume
model_time = Time_to_reach_bpt + time[1:num_steps] / 60
model_volume = 1000 * V_t_collected[1:num_steps]
for i, (exp_t, exp_v) in enumerate(zip(exp_interval_times, exp_interval_vol)):
    # Model volume at the experimental sampling time
    model_v = np.interp(exp_t, model_time, model_volume)
    plt.vlines(exp_t, ymin=min(exp_v, model_v), ymax=max(exp_v, model_v), colors='black', linewidth=1.5,
        linestyles='--', zorder=1, label='Model-experiment deviation' if i == 0 else None)
plt.xlabel('Distillation Time (min)', fontsize=12)
...
plt.scatter(exp_cum_times, exp_cum_conc,color='black', s=70, marker='o',zorder=3, label='Experimental')
plt.plot(Time_to_reach_bpt + time[1:num_steps]/60, C_e_collected[1:num_steps], 
         label='Model',zorder=2, linewidth=2, color='red')
# Model time and cumulative concentration
model_time = Time_to_reach_bpt + time[1:num_steps] / 60
model_conc = C_e_collected[1:num_steps]
# Model–experiment deviation
for i, (exp_t, exp_c) in enumerate(
        zip(exp_interval_times, exp_sample_conc_cumu)):
    # Model concentration at experimental sampling time
    model_c = np.interp(exp_t, model_time, model_conc)
    plt.vlines(exp_t, ymin=min(exp_c, model_c), ymax=max(exp_c, model_c), colors='black',
        linewidth=1.5, linestyles='--', zorder=1, label='Model-experiment deviation' if i == 0 else None)
plt.xlabel('Distillation Time (min)', fontsize=12)
plt.ylabel('Cumulative Distillate Ethanol Concentration (v/v%)', fontsize=12)
...
plt.plot(time_minutes, T_b[1:num_steps], linewidth=2, color='red', zorder=2)
plt.xlabel('Distillation Time (min)', fontsize=12)
