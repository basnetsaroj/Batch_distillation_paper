...
Initial_volume      = 3.5           # Initial volume (L)
Total_testing_time  = 53            # Total testing time (min)
Time_to_reach_bpt   = 15+40/60      # minutes
T_in_ref    = (23.4+25.3+25.5)/3    # 3 data points' average
T_out_ref   = (35+36.8+39.5)/3   
V_w_ref     = 900/((82+92+104)/3)   # ml/sec
FM_initial_temp     = 17            # Initial feed mixture temperature (Celsius)
C_ethanonl_initial  = 30            # Initial ethanol concentration (v/v)%   
...
T_room  = 18                # Room temperature (Celsius)
...
RR = H_water_ref/(PowerI - H_water_ref)                         # reflux ration, RR
...
for step in range(1, num_steps):
    ...
    no_dist = Energy / ((1 + RR) * latent_heat)     # Moles of distillate
